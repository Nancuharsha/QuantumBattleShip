    ...
    for player in range(2):
        ...
        for position in positions:
          #looping over the bombing
            for n in range( bombingTorpedo[(player+1)%2][position] ):
            #This is where we decrease the value of the ship stability using rotation function using “U3” method over one axis
                for ship in ships:
                    ...
                    if ( position == battleShipsLocations[player][ship] ):
                        frac = 1/(ship+1)
                        quantumCircuit[player].u3(frac * math.pi, 0.0, 0.0, quantumBits[position])
        #measuring the qbits and appending results in the classical registers                               
        for position in positions:
            quantumCircuit[player].measure(quantumBits[position], classicalBits[position])
